...
#define PLAY_IO_THRESHOLD_ACCELEROMETER 7000
#endif
...
static __NO_RETURN void demo (void *argument) {
  uint32_t  timestamp, timestamp_first, tick;
  uint32_t  n, size;
  uint16_t *pbuf;
  double    abs;
  double    abs_max;
  ...
  for (;;)  {
    ...
    size = sdsPlayRead(playId_accelerometer, &timestamp_first, tempBuf, size);
    ...
    for(;;) {
      ...
      osDelayUntil(tick + (timestamp - timestamp_first));
    }
  }
}
